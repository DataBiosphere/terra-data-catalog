...
def create_dataset(env, accessToken, storage_system, storage_system_id, entry):
    ...
    response = requests.post(
        f"https://catalog.dsde-{env}.broadinstitute.org/api/v1/datasets", headers=headers, data=json.dumps(request)
    )
    print(response.text)
    return None
...
def main(args, current_user):
    ...
    with open(args.metadata_file, 'r') as f:
        data = json.load(f)
    create_dataset(args.environment, access_token, args.storage_system, args.storage_system_id, data)
...
if __name__ == "__main__":
    current_user = subprocess.run(['gcloud', 'config', 'get-value', 'account'], check=True, capture_output=True).stdout.strip().decode("utf-8")
    try:
