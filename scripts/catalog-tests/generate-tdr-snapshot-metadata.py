...
urlDataRepoList = (
    os.environ.get("DATA_REPO_URL")
    or "https://jade.datarepo-dev.broadinstitute.org/api/repository/v1/snapshots"
)
...
def updateMetadataInCatalog(resourceJson, accessToken):
    ...
    for resource in resourceJson:
        existsResponse = requests.get(
            f"{urlDataRepoList}?filter={resource}", headers=headers
        )
...
def updateDataset(datasetId, datasetTitle, metadata, accessToken):
    ...
    response = requests.put(
        f"{urlDatasets}/{datasetId}", headers=headers, data=json.dumps(metadata)
    )
    logResponse(
        response, f"updating dataset with metadata for ({datasetId}, {datasetTitle})"
    )
...
def createDataset(datasetId, datasetTitle, metadata, accessToken):
    ...
    logResponse(
        response, f"creating dataset with metadata for ({datasetId}, {datasetTitle})"
    )
